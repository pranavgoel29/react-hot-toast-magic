...
✨ [toast.custom()](#introducing-toastcustom) - Dispatch components as toast
👀 [Reduced-motion support](#better-accessibility) - Automatically adapts to system preference
🔀 [Individually position toasts](#per-toast-positioning) 
🧈 [Smoother exit animations](#smoother-exit-animation) 

⚙️ [Custom Renderer API](#custom-renderer-api) - Supply your own render function

As well as many [other improvements and fixes](#changelog).
...
From now on, it's possible to have toasts at multiple positions at once. Just add the `position` you want as an option when dispatching a toast.
...
This is a great alternative if you are using [`useToaster()`](/docs/use-toaster) to create custom notifications.
